...
the herdr/Neovim boundary, and the assumption that an agent is doing the work.
...
Two hooks ship in the [`claude/`](../claude/.claude) package. Neither is part
of *using* the terminal — they're automation running behind it, and each gets a
document because the interesting part is why it works the way it does.

| | |
|---|---|
| **[claude-code.md](claude-code.md)** | Subagents and background shells as herdr tabs. Why Claude Code's own subagents are invisible to a multiplexer, the live file and hook events that make them visible anyway, and the rule that sends heavyweight subtasks to a real agent in a real tab. |
| **[cover-sync.md](cover-sync.md)** | Keeping `images/cover.png` in step across this repo, a local folder, and the GitHub profile. Why a newest-wins mirror needs two guards, and why the profile README carries a `?v=` on the image URL. |
...
then [claude-code.md](claude-code.md) for the agent side.
